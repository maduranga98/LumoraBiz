...
const RoutesPlanning = () => {
  ...
  // Day type options
  const dayTypes = [
    { value: 'working', label: 'Working Day', color: 'bg-green-100 text-green-800' },
    { value: 'mercantile_holiday', label: 'Mercantile Holiday', color: 'bg-orange-100 text-orange-800' },
    { value: 'holiday', label: 'Holiday', color: 'bg-red-100 text-red-800' },
    { value: 'leave', label: 'Leave', color: 'bg-yellow-100 text-yellow-800' },
    { value: 'sunday', label: 'Sunday', color: 'bg-purple-100 text-purple-800' },
    { value: 'company_holiday', label: 'Company Holiday', color: 'bg-blue-100 text-blue-800' }
  ];
  ...
  const fetchMonthlyPlan = async () => {
    ...
    setLoading(true);
    try {
      const planId = `${selectedYear}-${String(selectedMonth + 1).padStart(2, '0')}`;
      const planRef = doc(
        db,
        'owners',
        currentUser.uid,
  ...
  const handleDayTypeChange = (day, dayType) => {
    const isNonWorking = ['mercantile_holiday', 'holiday', 'leave', 'sunday', 'company_holiday'].includes(dayType);
  ...
  const getRouteName = (routeId) => {
    ...
    const route = selectedRepData.assignedRoutes.find(r => r.routeId === routeId);
  ...
  return (
    <div className="max-w-7xl mx-auto p-6">
      ...
      <div className="mb-8">
        ...
        <p className="text-gray-600">Plan monthly routes for your sales representatives</p>
      </div>
      ...
      <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-6 mb-6">
        ...
        {selectedRepData?.assignedRoutes && selectedRepData.assignedRoutes.length > 0 && (
          <div className="mt-6 pt-6 border-t border-gray-200">
            <h3 className="text-sm font-medium text-gray-700 mb-3">Assigned Routes:</h3>
        ...
        <div className="bg-white rounded-lg shadow-sm border border-gray-200 overflow-hidden">
          <div className="p-6 border-b border-gray-200 flex items-center justify-between">
            <h2 className="text-lg font-semibold text-gray-800">
              Monthly Plan - {selectedRepData?.name} ({getMonthName(selectedMonth)} {selectedYear})
            </h2>
            <button
              ...
              <table className="w-full">
                ...
                <tbody className="bg-white divide-y divide-gray-200">
                  {Array.from({ length: daysInMonth }, (_, index) => {
                    ...
                    return (
                      <tr key={day} className={`${isSunday ? 'bg-purple-50' : ''} ${isLocked ? 'opacity-60' : ''}`}>
                        {/* Date */}
                        <td className={`px-4 py-3 text-sm font-medium ${isSunday ? 'text-purple-900' : 'text-gray-900'}`}>
                          {day}
                        </td>

                        {/* Day Name */}
                        <td className={`px-4 py-3 text-sm ${isSunday ? 'text-purple-700 font-medium' : 'text-gray-600'}`}>
                          {dayName}
                        </td>
                        ...
                        <td className="px-4 py-3">
                          <select
                            value={dayPlan.dayType || (isSunday ? 'sunday' : 'working')}
                            onChange={(e) => handleDayTypeChange(day, e.target.value)}
                          ...
                          {dayPlan.dayType && (
                            <span className={`inline-block mt-1 px-2 py-1 rounded-full text-xs font-medium ${getDayTypeStyle(dayPlan.dayType)}`}>
                              {dayTypes.find(t => t.value === dayPlan.dayType)?.label}
                            </span>
                          )}
                        </td>
                        ...
                        <td className="px-4 py-3">
                          {isLocked ? (
                            ...
                            <select
                              ...
                              onChange={(e) => handleRouteSelection(day, e.target.value)}
                              className="w-full px-2 py-1 text-sm border border-gray-300 rounded focus:ring-2 focus:ring-blue-500 focus:border-blue-500"
                              disabled={!selectedRepData?.assignedRoutes || selectedRepData.assignedRoutes.length === 0}
                            >
      ...
      <div className="mt-6 bg-white rounded-lg shadow-sm border border-gray-200 p-6">
        <h3 className="text-sm font-medium text-gray-700 mb-3">Day Type Legend:</h3>
