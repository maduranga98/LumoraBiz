...
// Move components outside main component to prevent recreation on each render
        ...
        {Icon && (
          <Icon
            className={`absolute left-3 ${
              rows ? "top-3" : "top-1/2 transform -translate-y-1/2"
        ...
        {rows ? (
          <textarea
            ...
            className={`block w-full ${
              Icon ? "pl-10" : "pl-3"
            } pr-3 py-2 sm:py-3 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 text-sm sm:text-base resize-none`}
          ...
          <input
            ...
            className={`block w-full ${
              Icon ? "pl-10" : "pl-3"
            } pr-3 py-2 sm:py-3 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 text-sm sm:text-base`}
            placeholder={placeholder}
            required={required}
            autoComplete="off"
          />
...
const Modal = React.memo(({ isOpen, onClose, title, children, isMobile }) => {
...
const ModalActions = React.memo(
  ({ onSubmit, submitText, onCancel, submitting }) => (
...
const MobileBuyerCard = React.memo(
  ({ buyer, openEditModal, openDeleteModal }) => (
...
export const Buyers = () => {
  ...
  // Handle input changes - Fixed to prevent re-renders
  ...
  }, [currentBusiness, resetForm]);

  // Open edit modal
  const openEditModal = useCallback((buyer) => {
  ...
  // Open delete modal
  const openDeleteModal = useCallback((buyer) => {
    setCurrentBuyer(buyer);
